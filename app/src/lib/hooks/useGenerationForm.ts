...
export function useGenerationForm(options: UseGenerationFormOptions = {}) {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      // Qwen CustomVoice uses preset-voice "instruct" controls; OmniVoice
      // uses "instruct" for voice design (gender/accent/dialect). The
      // backend enforces English/Chinese for OmniVoice's voice design
      // (per upstream docs), so we just forward whatever the user typed.
      const supportsInstruct = engine === 'qwen_custom_voice' || engine === 'omnivoice';
